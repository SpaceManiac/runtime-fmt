...
// modified here to replace standard macros with this crate's macros
// modified below to add #[test] and comment out calling inner testcases
// begin modifications
...
macro_rules! println {
    ($($rest:tt)*) => { rt_println!($($rest)*).unwrap() }
}
macro_rules! write {
    ($($rest:tt)*) => { rt_write!($($rest)*).unwrap() }
}
macro_rules! writeln {
    ($($rest:tt)*) => { rt_writeln!($($rest)*).unwrap() }
}
// end modifications
